...
while(1)
{
	...
	{	
		if (string(cmdTokens[j]).find("&&") == std::string::npos)
		{
			...
			char *cmdToExec[separatedCmdTokens.size()+1];
			for (int i = 0; i < separatedCmdTokens.size(); i++)
			{
			...
			|| string(cmdTokens[j]).find("||") != std::string::npos)
		{
			...
			for (int k = 0; k < andCmdTokens.size(); k++)
			{
				...
				char *cmdToExec[andSeparatedCmdTokens.size()+1];
				for (int i = 0; i < andSeparatedCmdTokens.size(); i++)
				{
